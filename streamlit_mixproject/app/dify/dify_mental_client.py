...
class DifyArticleClient:
    ...
    def __init__(self):
        self.api_key = settings.DIFY_MENTAL_API_KEY
        self.workflow_id = settings.DIFY_MENTAL_WORKFLOW_ID
                        ...
                        async for line in response.aiter_lines():  #异步迭代器，异步逐行读取响应内容的方法
                            if line.startswith("data: "): #只有以 "data: " 开头的行才是有效的事件数据
                                try:
                                    ...
                                    if event_data.get("event") == "message":  #检查解析后的数据中 event 字段是否为 "text_chunk"
...
# 创建全局客户端实例
dify_mental_client = DifyArticleClient()
