...
from app.dify.dify_article_client import dify_article_client
from app.schemas.dify_article import ArticleResponse, ArticleCreate

dify_article = APIRouter(prefix="/articles", tags=["文章生成"])
...
async def generate_article(
    ...
    try:
        inputs = {"topic": article_data.topic}
        result = await dify_article_client.run_workflow(
            inputs=inputs,
...
async def generate_article_stream(
    ...
    async def event_generator():
        ...
        try:
            ...
            # 异步迭代：从Dify客户端获取流式响应
            # 每次迭代获得一个文本块(chunk)
            async for chunk in dify_article_client.run_workflow_streaming(inputs=inputs,user_id=article_data.user_id):
